...
export default function App() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const r = await window.storage.get(SAVE_KEY, true);
  ...
  const saveData = useCallback(async (d) => { setSaving(true); try { await window.storage.set(SAVE_KEY, JSON.stringify(d), true); } catch {} setSaving(false); }, []);
